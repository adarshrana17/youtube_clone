...
function Login({ sidebar, setSidebar }) {
  ...
  const categories = [
    ...
    "News", "Cartoon", "Entertainment", "Sports", "Dubbing", "Coding", "Podcast",
    "Live", "Football", "React"
  ];
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        ...
        const response = await fetch("http://localhost:5100/loggedin-user", {
          method: "GET",
  ...
  return (
    <>
      <Header setSidebar={setSidebar} sidebar={sidebar} setFilteredVideos={setFilteredVideos} showModal={true} custom_css="py-2.5"/>
      {sidebar && <Sidebar />}

      {/* Main Content */}
      <div className={`transition-all duration-300 ${sidebar ? "md:w-[calc(100%-230px)] ml-auto -z-50" : "w-full"}`}>
            ...
            {categories.map((category, index) => (
              <div 
                ...
                className={`px-4 py-2 rounded-lg cursor-pointer hover:bg-gray-200 transition-all duration-200 
                  ${selectedCategory === category ? "bg-gray-300 font-bold" : "bg-gray-100"}`}
              >
                {category}
              </div>
            ))}
        ...
        <div className={`grid gap-4 p-4 mt-[115px] w-full 
            ${sidebar ? "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4" : "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"}`}>
          {filteredVideos.length > 0 ? (
            filteredVideos.map((data) => <VideoItem key={data.id} videoDetails={data} />)
          ) : (
